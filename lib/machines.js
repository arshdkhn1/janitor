...
exports.spawn = function (user, projectId, callback) {
  ...
  const machine = createNewMachine(user, projectId);
...
exports.destroy = function (user, projectId, machineId, callback) {
  ...
  // XXX: remove check for status once db has been cleaned
  if (!containerId || machine.status === 'new') {
    // This machine has no associated container, remove it
    machines.splice(machineId, 1);
    db.save();
    callback();
    return;
  }
...
function createNewMachine (user, projectId) {
  const project = getProject(projectId);
  if (!project) {
