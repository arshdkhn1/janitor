...
// Get an existing user machine given the container ID and the hostname.
exports.getMachineByContainer = function (user, containerId, hostname) {
  ...
  const machine = user.machines[containerId];
  
  if (machine) {
    if (!hostname || machine.docker.host === hostname) {
      return machine;
    }
  }
...
exports.spawn = function (user, projectId, callback) {
  ...
  const machine = createNewUserMachine(user, projectId);
...
// Create a new user machine for a project.
function createNewUserMachine (user, projectId) {
  const project = getProject(projectId);
  if (!project) {
