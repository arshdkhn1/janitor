...
function getOrCreateUser (email) {
  ...
  if (!user.usesNewMachines) {
    let newMachines = {};
    // Temporary migration code: Previous users have a different machines object.
    for (const projectId in user.machines) {
      for (const machine of user.machines[projectId]) {
        if (machine.status === "new" || !machine.docker
            || !machine.docker.container) {
          continue;
        }

        // Temporary migration code: much older users didn't have machine properties.
        if (!machine.properties) {
          machine.properties = { name: machine.name };
          delete machine.name;
        }
        machine.project = projectId;
        newMachines[machine.docker.container] = machine;
      }
    }
  
    user.machines = newMachines;
    user.usesNewMachines = true;
  }
